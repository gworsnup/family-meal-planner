...
export default function FeaturesHomeOne({
  ...
  const defaultItems: (FeatureItem & { icon: ReactNode })[] = [
    {
  ...
  const title = content?.title ?? "Core features that make it valuable";
        ...
        <div className="azzle-iconbox-wrapper" data-aos="fade-up" data-aos-delay="700">
          <div className="azzle-iconbox-column">
            {items.map((item, index) => (
              <div
                key={item.title}
                className={`azzle-iconbox-wrap${index === 3 ? " wrap2" : ""}`}
              >

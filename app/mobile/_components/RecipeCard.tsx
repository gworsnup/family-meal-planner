...
export default function RecipeCard({ slug, recipe }: RecipeCardProps) {
  ...
  return (
    <Link href={`/mobile/${slug}/recipes/${recipe.id}`} className="flex gap-4 rounded-2xl border border-slate-200 bg-white p-3 transition hover:border-slate-300 hover:shadow-sm active:scale-[0.99]">
      {recipe.photoUrl ? (

...
export default async function MobileRecipePage({
  ...
  return (
    <article className="space-y-5">
      ...
      {saved ? <p className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-800">Recipe saved to your library.</p> : null}

      {recipe.photoUrl ? (
        // Imported recipe images can be hosted on arbitrary source domains.
        // eslint-disable-next-line @next/next/no-img-element
        <img src={recipe.photoUrl} alt="" referrerPolicy="no-referrer" className="h-64 w-full rounded-3xl object-cover" />
      ) : null}

      <div>
        <h1 className="text-3xl font-bold leading-tight tracking-tight">{recipe.title}</h1>
      ...
      <Link href={`/mobile/${slug}/recipes/${recipe.id}/cook`} className="block rounded-2xl bg-slate-900 px-5 py-4 text-center text-base font-bold text-white">Start cooking</Link>
        ...
        {recipe.ingredientLines.length ? (
          <ul className="mt-4 space-y-3">
            {recipe.ingredientLines.slice().sort((a, b) => a.position - b.position).map((line) => (
              <li key={line.id} className="flex gap-3 text-base leading-6 text-slate-700"><span className="text-emerald-600">●</span><span>{line.ingredient}</span></li>
            ))}
          </ul>

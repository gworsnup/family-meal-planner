...
export default async function MobilePlanPage({
  ...
  return (
    <div className="space-y-5">
      <div>
        <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Meal plan</p>
        <h1 className="mt-2 text-2xl font-semibold text-slate-900">This week</h1>
      </div>
      ...
      <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-2">
        ...
        <div className="text-center"><p className="text-sm font-semibold">{formatWeekRange(weekStart)}</p><Link href="?" className="text-xs font-semibold text-slate-500">Jump to current week</Link></div>
      ...
      <div className="space-y-3">
        {days.map((day, index) => {
          ...
          return (
            <section key={dateISO} className={`overflow-hidden rounded-2xl border bg-white ${isToday ? "border-slate-400 shadow-sm" : "border-slate-200"}`}>
              <div className="flex items-center justify-between px-4 py-3">
                <div className="flex items-center gap-2"><h2 className="font-semibold">{DAY_LABELS[index]}</h2>{isToday ? <span className="rounded-full bg-slate-900 px-2 py-0.5 text-[10px] font-semibold uppercase text-white">Today</span> : null}</div>
              ...
              {dayItems.length ? <div className="border-t border-slate-100">{dayItems.map((item) => {
                ...
                return <Link key={item.id} href={`/mobile/${slug}/recipes/${item.recipe.id}`} className="flex items-center gap-3 px-4 py-3 active:bg-slate-50">{item.recipe.photoUrl ? <>
                ...
                </> : <div className="flex h-16 w-16 flex-none items-center justify-center rounded-xl bg-slate-100 text-2xl">🍲</div>}<div className="min-w-0 flex-1"><p className="font-semibold leading-5">{item.recipe.title}</p><p className="mt-1 text-xs font-semibold text-slate-500">View recipe</p></div><span className="text-2xl text-slate-300">›</span></Link>;
